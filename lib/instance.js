...
module.exports = (function() {
  ...
  Instance.prototype.get = function(key, options) {
    ...
    if (key) {
      ...
      else if (options && this.options && this.options.include && this.options.includeNames.indexOf(key) !== -1) {
        var included = this.dataValues[key];
        if (Array.isArray(included)) {
          return included.map(function (value) { 
    ...
    var hasInclude = this.options && this.options.include && this.options.includeNames;
    for (_key in this.dataValues) {
      if (!values.hasOwnProperty(_key) && this.dataValues.hasOwnProperty(_key)) {
        if (hasInclude && this.options.includeNames.indexOf(_key) !== -1) {
          var included = this.dataValues[_key];
          if (Array.isArray(included)) {
            values[_key] = included.map(function (value) {
              return value.get(options);
            });
          }
          else {
            values[_key] = included.get(options);
          }
        }
        else {

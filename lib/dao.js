...
module.exports = (function() {
  ...
  DAO.prototype.get = function (key, options) {
    ...
    if (key) {
      if (this._customGetters[key]) {
        ...
        if (this.options && this.options.include && this.options.includeNames.indexOf(key) !== -1) {
          var included = this.dataValues[key]
          if (Array.isArray(included)) {
            return included.map(function (value) { return value.get(options) })
          }
          else {
            return included.get(options)
          }
        }//fall-through: not DAO
      }//fall-through: not plain
    ...
    var hasIncludes = this.options && this.options.include && this.options.includeNames;
    for (key in this.dataValues) {
      if (!values.hasOwnProperty(key) && this.dataValues.hasOwnProperty(key)) {
        var included = values[key] = this.dataValues[key]
        if (options.plain) {
          if (hasIncludes && this.options.includeNames.indexOf(key) !== -1) {
            if (Array.isArray(included)) {
              values[key] = included.map(function (value) { return value.get(options) })
            }
            else {
              values[key] = included.get(options)
            }
          }
        }
      }

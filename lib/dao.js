...
module.exports = (function() {
  ...
  DAO.prototype.get = function (key, options) {
    ...
    if (key) {
      if (this._customGetters[key]) {
        ...
        if (this.dataValues[key] instanceof DAO) {
          // get the member plainly if requested
          return this.dataValues[key].get(options)
        }
        else if (Array.isArray(this.dataValues[key]) && this.dataValues[key].length && this.dataValues[key][0] instanceof DAO) {
          // reduce an array of DAOs
          return this.dataValues[key].map(function (value) {
    ...
    if (this._hasCustomGetters) {
      for (key in this._customGetters) {
        if (this._customGetters.hasOwnProperty(key)) {
          values[key] = this.get(key, options)
        }
      }
    }
    else if (!options.plain) {
    ...
    for (key in this.dataValues) {
      if (!values.hasOwnProperty(key) && this.dataValues.hasOwnProperty(key)) {
        ...
        if (options.plain) {
          if (originalValue instanceof DAO) {
            // reduce individual member if plain
            values[key] = originalValue.get(options)
          }
          else if (Array.isArray(originalValue) && originalValue.length && originalValue[0] instanceof DAO) {
            // reduce arrays if requested plain
            values[key] = originalValue.map(function (value) { return value.get(options) })
          }
          else {
